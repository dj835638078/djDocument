...
export default {
    ...
    beforeRouteLeave(to, from ,next) {
        localStorage.removeItem("WalkRouteInfoListStorage")
        next()
    },
    methods: {
        getData(){
            var self = this
            self.list = JSON.parse(localStorage.getItem("WalkRouteInfoListStorage"))
        ...
        errorBtn(fileurl){ 
            mapDataReport("ugcreport_walk_trace")
            this.fileurl = fileurl
            this.$router.push({path:'/walk-navigation',query:{fileurl:this.fileurl}}); 
        },
    },
}
</script>
