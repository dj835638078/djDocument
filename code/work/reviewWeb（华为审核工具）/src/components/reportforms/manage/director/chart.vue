  ...
  <div class="chart">
    <div v-if="chartList">
      <div class="chart-box">
        ...
        <div
          class="chart-box-title-b"
          v-if="chartList.length && washLabel != 'yes'"
        >
          审核数
        </div>
        <div
            ...
            <ul class="manpower-content-list">
              <li class="manpower-list" v-for="(item, i) in chartList" :key="i">
                <div class="manpower-list-name ellipsis">
                  <span v-if="chartTitle == '团队'">
                    <el-tooltip
                      placement="top"
                      effect="dark"
                      :open-delay="200"
                      :visible-arrow="false"
                      v-if="item.regionName && item.regionName.length > 4"
                    >
                      <span>{{ item.regionName }}</span>
                      <div slot="content">{{ item.regionName }}</div>
                    </el-tooltip>
                    <span v-else>{{ item.regionName }}</span>
                  </span>

                  <span v-if="chartTitle == '个人'">
                    <el-tooltip
                      placement="top"
                      effect="dark"
                      :open-delay="200"
                      :visible-arrow="false"
                      v-if="item.processName && item.processName.length > 4"
                    >
                      <span>{{ item.processName }}</span>
                      <div slot="content">{{ item.processName }}</div>
                    </el-tooltip>
                    <span v-else>{{ item.processName }}</span>
                  </span>

                  <span v-if="labelName == '清洗分类数量'">
                    <el-tooltip
                      placement="top"
                      effect="dark"
                      :open-delay="200"
                  ...
                  <span v-if="labelName == '标注数量'"
                    ...
                    <span v-else>{{ item.regionName }}</span></span
                  >
                </div>
                <div class="manpower-list-size">
                  ...
                  >
                    ...
                    ></div>
                    <div v-if="chartTitle == '团队'" slot="content">
                      审核员： {{ item.regionName }}<br /><br />审核数量：
                      {{ item.processCount }}条
                    </div>
                    ...
                    <div v-if="labelName == '清洗分类数量'" slot="content">
                      清洗小组： {{ item.regionName }}<br /><br />清洗数量：
                      {{ item.processCount }}条
                    </div>
                    <div v-if="labelName == '标注数量'" slot="content">
                      标注小组： {{ item.regionName }}<br /><br />标注数量：
                      {{ item.processCount }}条
                    </div>
                  </el-tooltip>
                </div>
...
export default {
  ...
  props: ["chartList", "numList", "chartTitle", "labelName", "washLabel"],
  mounted() {
    this.monitor();
