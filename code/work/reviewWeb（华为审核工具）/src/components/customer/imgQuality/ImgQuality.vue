  ...
  <div class="ImgQuality">
    <div class="tabBox">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="巡检内容" name="1" style="color:red;">
        </el-tab-pane>
        ...
        <div class="search-box-top-right">
          <div class="one-box">
            <div class="one-box-child">
              <el-select v-model="dataType1" placeholder="请选择巡检类型">
                <el-option
    ...
    <div v-if="infoDialogFlag">
      <infoDialog :dataInfo="detailList" :title="title" />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    query() {
      ...
      api
        .post(
          `./manage/service/graphicsClientCheck/queryByVgc?currentPage=${this.currentPage}&pageSize=${this.pageSize}`,
          data
        )
    ...
    casual() {
      let data = {
        //dataType: this.dataType3
        dataSource: this.dataType3,
        lastProcessDate: this.processDate,
        lastEndProcessDate: this.endProcessDate,
        lastProcessResult: this.processResult,
        accountId: this.accountId,
        ownerAccount: this.processAccount
      };
      let vm = this;
      api
        .post(`./manage/service/graphicsClientCheck/add`, data)
        .then(function(response) {
          if (response.status == 200) {
            vm.currentPage = 1;
            vm.pageSize = 10;
    ...
    nextBatch() {
      this.$confirm(
        ...
        .then(() => {
          ...
          let vm = this;
          api
            .post(`./manage/service/graphicsClientCheck/release`, data)
            .then(function(response) {
              if (response.status == 200) {
                vm.currentPage = 1;
                vm.pageSize = 10;
    ...
    inquiry() {
      ...
      api
        .post(
          `./manage/service/graphicsClientCheck/query?currentPage=${this.currentPage}&pageSize=${this.pageSize}`,
          data
        )
        .then(function(response) {
          if (response.status == 200) {
    ...
    decideOne() {
      if (this.dataType1) {
        ...
        api
          .get(
            `./manage/service/channelLookUp/findLookUps?type=1_1&level=2&code=${this.dataType1}`
          )
          .then(function(response) {
    ...
    decideTwo() {
      if (this.dataType2) {
        ...
        api
          .get(
            `./manage/service/channelLookUp/findLookUps?type=1_1&level=3&code=${this.dataType2}`
          )
          .then(function(response) {
