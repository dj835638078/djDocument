<template>
  <div class="AuditTaskAuto">
    <!--删除弹框-->
    <Remove event="removeEvent" />
    <!--操作组-->
    <div class="btnList">
      <div class="new" @click="newRule">新建规则</div>
    ...
    <div class="queryList">
      <el-form :inline="true" :model="formQuery" class="demo-form-inline">
        <div class="formItemList">
          <span class="specialSpan" style="margin-left:24px">适用范围</span>
          <el-form-item>
            <el-select
          ...
          <el-form-item class="queryBtn">
            <div class="query" @click="researchForm">
              <i class="fa fa-search"></i>查询
            </div>
            <div class="query" @click="resetForm">
    ...
    <div class="tableBox">
      ...
      >
        ...
        <el-table-column fixed="left" label="操作" width="66" align="left">
          <template slot-scope="scope">
            <div type="text" size="small" title="编辑" @click="edit(scope.row)">
            ...
            >
              <i class="fa fa-wpforms settingCol"></i>
            </div>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <div @click="lookCli(scope.row)" class="ruleName">
              <i
                v-if="scope.row.autoState == 1"
              ...
              <i
                title="已禁用"
                v-if="scope.row.autoState == 0"
                class="fa fa-ban"
                style="font-size:14px;color:#f56c6c;margin-right:10px;line-height:20px;"
              ></i
              ><span style="color:#1890FF">{{ scope.row.autoName }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="dataSourceName" label="规则类型" min-width="100">
        </el-table-column>
        <el-table-column
        ...
        <el-table-column label="操作人" align="left" width="120">
          <template slot-scope="scope">
            <div v-text="tools.setRole(scope.row.lastUpdateBy)"></div>
          </template>
        </el-table-column>
        <el-table-column label="操作时间" align="left" width="160">
          <template slot-scope="scope">
            <span
...
export default {
  ...
  methods: {
    ...
    queryDaily(val) {
      this.$router.push({
        name: "AuditTaskDaily",
        params: { id: val.autoId }
      });
    },
    edit(val) {
      this.$router.push({
        name: "AuditTaskAutoRule",
        params: { name: "edit", id: val.autoId }
      });
    },
    //新建
    newRule() {
    ...
    lookCli(val) {
      this.$router.push({
        name: "AuditTaskAutoRule",
        params: { name: "look", id: val.autoId }
      });
    },
    //禁用、启用
    prohibit(name) {
      ...
      if (vm.container.length != 0) {
        ...
        if (name == "start") {
          url = "./manage/service/taskAuto/turnOn";
          strName = "确定要启用" + num + "规则";
        } else {
          url = "./manage/service/taskAuto/turnOff";
          strName = "确定要禁用" + num + "规则";
        }
        if (vm.container.length == 1) {
          if (vm.container[0].autoState == 1 && name == "start") {
            vm.$message({
              showClose: true,
              message: "此条数据已启用",
              type: "warning"
          ...
          if (vm.container[0].autoState == 0 && name == "prohibit") {
            vm.$message({
              showClose: true,
              message: "此条数据已禁用",
              type: "warning"
        ...
        vm.$confirm(strName, "提示", {
          ...
          .then(() => {
            ...
            for (let i = 0; i < vm.container.length; i++) {
              // let obj = {};
              // obj.autoId = vm.container[i].autoId;
              arr.push(vm.container[i].autoId);
            }
            vm.api
              .post(url, arr)
              .then(function(response) {
    ...
    getRuleList(flag) {
      ...
      if (vm.formQuery.dataSource) {
        data.dataSource = vm.formQuery.dataSource;
      }
      ...
      vm.api
        .post(
          `./manage/service/taskAuto/findPage?currentPage=${
            vm.currentPage
...
.queryBtn /deep/.el-form-item__content {
  width: 100%;
}
.AuditTaskAuto {
