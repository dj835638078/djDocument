  ...
  methods: {
    ...
    getInfo() {
      let data = {
        ...
        startDate: this.createDateBottom,
        endDate: this.endDateBottom,
        processResult: this.status,
        rowKey: this.rowkey
      };
      let vm = this;
      vm.api
        .post(
          `./manage/service/videoVerify/verifyItems?currentPage=${this.currentPage}&pageSize=${this.pageSize}`,
          data
        )
    ...
    decideBottom() {
      if (this.dataTypeBottom1) {
        ...
        vm.api
          .get(
            `./manage/service/channelLookUp/findLookUpsAssgin?type=1_2&level=2&code=${this.dataTypeBottom1}`
          )
          .then(function(response) {
      ...
      if (this.dataTypeBottom2) {
        ...
        vm.api
          .get(
            `./manage/service/channelLookUp/findLookUpsAssgin?type=1_2&level=3&code=${this.dataTypeBottom2}`
          )
          .then(function(response) {
