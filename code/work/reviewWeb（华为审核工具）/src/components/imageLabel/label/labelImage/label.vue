  ...
  <div class="washDetail">
    ...
    <div class="tabBox">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane
          :label="$route.name == 'labelCheck' ? '未质检' : '未标注'"
          name="first"
        >
        </el-tab-pane>
        <el-tab-pane
          :label="$route.name == 'labelCheck' ? '已质检' : '已标注'"
          name="second"
        ></el-tab-pane>
      </el-tabs>
    ...
    <div class="content-box">
      ...
      <div
        class="infoHeadBtn"
        v-if="activeName == 'first' && $route.name != 'labelCheck'"
      >
        <div
          class="release"
          v-if="
        ...
        <div
          class="imgInfoOperate"
          v-if="activeName == 'first' && totals"
        ></div>
      </div>
      <div
        class="infoHeadBtn"
...
export default {
  ...
  methods: {
      ...
      if (this.$route.name == "labelImage") {
        ...
        if (this.imgList[this.imgIndex].imgId != this.imgListYN[0].imgId) {
          ...
          this.listS = JSON.parse(
            this.imgList[this.imgIndex].imgLabelCoordinate
          );
        } else {
          if (this.imgList[this.imgIndex].currentPage > 1) {
            this.currentPage = this.currentPage - 1;
    ...
    upLoadAddFile() {
      if (!this.selectFile) {
        this.$message.warning(`请选择要上传的分类`);
        return;
      }
      // if (this.selectFile) {
      //   this.actionUrl = `./manage/publicservice/label/image/upload?${+new Date().getTime()}&orderId=${
      //     this.$route.params.id
      //   }&imgType=2&fileClassify=${this.selectFile}`;
      // }
    },
    // 分类取消按钮
    classifyBtnN() {
    ...
    upLoadBtnN() {
      ...
      if (vm.uploadFlag) {
        ...
        for (var i = 0; i < vm.fileListOptinUpload.length; i++) {
          if (vm.fileListOptinUpload[i].classifyId == vm.selectFile) {
            dataObj.classifyName = vm.fileListOptinUpload[i].classifyName;
            vm.fileClassifyObj.classifyName =
              vm.fileListOptinUpload[i].classifyName;
          }
        }
        vm.fileClassifyId = vm.selectFile;
        dataObj.orderId = vm.$route.params.id;
        dataObj.classifyId = vm.selectFile;
        vm.fileClassifyObj.classifyId = vm.selectFile;
        vm.fileClassifyObj.orderId = vm.$route.params.id;
    ...
    // 提交按钮
    submitCli() {
      this.titleName = "您确定要提交到下一环节吗？";
      this.titleDesc = "提交表示您确认已质检内容";
    ...
    moveSelChoose(itema, item, indexa) {
      ...
      objObj.labelStatus = 0;
      objObj.flag = "label";
      if (this.$route.name == "labelCheck") {
        objObj.flag = "check";
        objObj.checkingStatus = "0";
      }
    ...
    operateBtnY() {
      ...
      if (this.operateFlag == "submit") {
        vm.api
          .get(
            `./manage/service/imglabel/labelCheck?orderId=${
              this.$route.params.id
            }`
          )
          .then(function(response) {
    ...
    checkboxChange(index, item) {
      ...
      if (item.isSel) {
        ...
        this.deleteArrObj.labelStatus = 0;
  ...
  components: { labelStory, labelImg },
  computed: {
    myHeaders() {
      return {
  ...
  mounted() {
    ...
    this.$route.matched[0].meta.title = "数据标注";
    if (this.$route.name == "labelCheck") {
