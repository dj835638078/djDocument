    ...
    <div class="task-search">
      <div class="task-search-box">
        <div class="task-search-top">
          ...
          <div class="space"></div>
          <div class="task-search-top-box" v-if="$route.name == 'washList'">
            <div class="task-search-top-box-name">清洗状态：</div>
            <div class="task-search-top-box-text">
              <el-select
                v-model="nodeState"
                placeholder="请选择订单状态"
              ...
              >
                <el-option
                  v-for="(item, index) in lookUpStatec"
                  :label="item.itemName"
                  :value="item.itemCode"
                  :key="index"
                ></el-option>
          ...
          >
            ...
            <div class="task-search-top-box-text">
              ...
              >
                <el-option
                  v-for="(item, index) in lookUpStateq"
                  :label="item.itemName"
                  :value="item.itemCode"
                  :key="index"
                ></el-option>
    ...
    <div style="margin-top:20px;">
      ...
      >
        ...
        <el-table-column
          ...
          label="操作"
          align="left"
          :width="$route.name == 'washList' ? '100' : '75'"
        ...
        >
          <template slot-scope="scope">
            ...
            <span
              type="text"
              size="small"
              :title="$route.name == 'washList' ? '清洗分类' : '清洗分类质检'"
        ...
        >
          <template slot-scope="scope">
            <span v-text="tools.setWashStatus(scope.row.nodeState)"></span>
          </template>
        </el-table-column>
        <el-table-column
          label="质检状态"
        ...
        >
          <template slot-scope="scope">
            <span v-text="tools.setLabelStatusC(scope.row.nodeState)"></span>
          </template>
        </el-table-column>
        <el-table-column
          prop="orderQuantity"
...
export default {
  ...
  components: {},
  computed: {
    ...mapState(["lookUp", "tableHeightThreeY", "lookUpStatec", "lookUpStateq"])
  },
  methods: {
    //判断时间
    judgeTime() {
    ...
    //查看详情
    goDetail(val) {
      let roleName = "operateWash";
      if (this.$route.name == "checkWashList") {
        roleName = "checkWashDetail";
      }
      this.$router.push({
        name: "orderDetail",
        params: {
    ...
    getTaskList() {
      ...
      if (this.$route.name == "checkWashList") {
        data.isCheck = "1";
        url = `./manage/service/materiai/pageCheck?currentPage=${
          this.currentPage
        }&pageSize=${this.pageSize}`;
      } else {
        url = `./manage/service/materiai/page?currentPage=${
          this.currentPage
        }&pageSize=${this.pageSize}`;
      }
