...
export default {
  ...
  imageLabel: function (c) {
    ...
    var $imageLabel = {
      getData: function () {
        var d = [];
        $('.imageLabel-imgdrop').each(function (index, item) {
        ...
        <div class="imageLabel-img-boxs">
            <span class="imageLabel-img-body">
                ...
                <div class="imageLabel-jisuan" style="position: relative;overflow:hidden;">
                    <img src="${c.img}" alt="" style='position: absolute;width:100%;height:100%;' class="imageLabel-img">
        ...
        <div class="imageLabel-input" style='background-color:rgba(255,255,255,0.3);'>
            <div class="imageLabel-input-box" style='width:250px;'>
                <div style='background-color: #333;'>
                    <div style='color:rgba(0, 0, 0, 0.85);overflow:hidden;line-height: 40px;background:#fff;'>
                        <span style='float: left;margin-left:20px;font-weight:600;'>编辑内容</span>
                        <span class="imageLabel-input-close" style='float:right;margin-right:20px;cursor: pointer;'>X</span>
                    </div>
                </div>
                <div style='background: #fff;padding:20px;'>
                    <input type="text"  placeholder="请输入内容" value='' max='10' style='width:100%;padding:5px;border:1px solid #D5D5D5'>
                    <div style='margin-top:20px;overflow:hidden;'>                                            
                    <div class="imageLabel-input-ok imageLabel-btn" style='float: left;width:65px;height:23px;background-color: #597EF7;line-height:23px;padding:0;border-radius:0px;font-size:12px;margin-left:40px;margin-right:8px;'>确定</div>
                    <div class="imageLabel-input-close imageLabel-btn" style='float: left;width:65px;height:23px;line-height:23px;background:#D9D9D9;padding:0;border-radius:0px;color:rgba(0, 0, 0, 0.65);font-size:12px;'>取消</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    function next() {
      ...
      $('.imageLabel-content').mousedown(function (e) {
        //2019.03.20
        if (config.judgeLabel() != 'down') {
          button = e.button;
          if (e.button != 2) { //左键点击事件
            //debugger
            //if ($imageLabel.getData().length < 2) {
            $menu.hide();
            flag = true;
            $t = $(this);
            obj = {
      ...
      $('.imageLabel-img-boxs').mousemove(function (e) {
        ...
        config.dataInit($imageLabel.getData())
      });
      ...
      $input.find('.imageLabel-input-ok').click(function () {
        if ($imageLabel.getData().length < 16) {
          //debugger
          ...
          if ($i.val()) {
            ...
            // removeClass("imageLabel-imgdrop")
          }
          //zx 代码
          config.dataInit($imageLabel.getData())
        } else {
      ...
      $("body").on("click", ".imageLabel-imgdrop", function () {

        config.getIndex(this, $imageLabel.getData())
