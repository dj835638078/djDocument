<template>
  <div class="washDetail">
    <div style="height:42px;padding:20px 20px 10px 20px;">
      <div class="video-box">
        <div class="video-box-name">标注分类：</div>
      ...
      <div class="video-box">
        <div class="video-box-name">标注人员：</div>
        <div class="video-box-select">
    ...
    <div style="margin-top:10px;">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span v-text="tools.setRole(scope.row.firstLabelAccount)"></span>
          </template>
        </el-table-column>

        <el-table-column label="标注时间" align="left">
          <template slot-scope="scope">
            <span
              v-text="tools.setFormTime(scope.row.firstLabelDate, 1)"
            ></span>
          </template>
        </el-table-column>
      </el-table>
...
<script>
import labelImg from "./../labelImage/labelImg";
...
export default {
  ...
  methods: {
    ...
    viewImg(val, index) {
      ...
      this.imgS =
        ...
        `/manage/publicservice/label/image/download?fId=${
          this.imgList[index].imgId
        }&imgType=2`;
      this.listS = JSON.parse(val.imgLabelCoordinate);
      this.imgShow = true;
    },
    //点击向左
    goLeft() {
      //判断储存变量是否和大变量相同来判断是不是第一位
      if (this.imgList[this.imgIndex].imgId != this.tableData[0].imgId) {
        //不是第一位时,向前一位
        ...
        if (this.imgList[this.imgIndex].currentPage > 1) {
          ...
          let data = {
            ...
            firstLabelAccount: this.createBy,
            fileClassify: this.fileClassify
          };

          this.api
            .post(
              `./manage/service/imglabel/query?currentPage=${
                this.currentPage
            ...
            .then(function(response) {
              if (response.status == 200) {
                ...
                for (let i = 0; i < vm.tableData.length; i++) {
                  let obj = {};
                  obj.imgId = vm.tableData[i].imgId;
                  obj.fileName = vm.tableData[i].fileName;
                  obj.currentPage = vm.currentPage;
                  obj.imgLabelCoordinate = vm.tableData[i].imgLabelCoordinate;
                  vm.imgList.push(obj);
                }
    ...
    goRight() {
      if (
        this.imgList[this.imgIndex].imgId !=
        this.tableData[this.tableData.length - 1].imgId
      ) {
        this.imgIndex = this.imgIndex + 1;
        this.imgS =
          location.origin +
    ...
    downLoad() {
      ...
      location.href =
        location.origin +
        `/manage/publicservice/imglabel/imglabelDown?orderId=${
          this.$route.params.id
        }&fileClassify=${this.fileClassify}&firstLabelAccount=${
          this.createBy
        }&start=${timea}&end=${timeb}`;
    },
    //分页
