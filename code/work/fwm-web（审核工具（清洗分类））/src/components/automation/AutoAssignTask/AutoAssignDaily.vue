<template>
  <div class="AutoAssignDaily">
    <!--操作组-->
    <div class="btnList">
      <div class="new" @click="backCli"><i class="fa fa-reply"></i>返回</div>
    </div>
    <!--表格-->
    <div class="tableBox">
      <el-table
        ...
        :height="tableHeight"
      >
        <el-table-column prop="operateType" label="操作" width="60">
        </el-table-column>
        <el-table-column
          label="规则名称"
          prop="assignName"
        ...
        <el-table-column prop="fwmAccount" label="账号名称" min-width="100">
          <template slot-scope="scope">
            <div v-text="tools.setRole(scope.row.fwmAccount)"></div>
          </template>
        </el-table-column>
        <el-table-column label="规则状态" width="70">
          <template slot-scope="scope">
            <div
              :class="[
                scope.row.assignState == '1' ? 'activeColorY' : 'activeColorN'
              ]"
              v-text="scope.row.assignState == '1' ? '启用' : '禁用'"
            ></div> </template
        ></el-table-column>
        <el-table-column
          prop="dataSourceName"
...
export default {
  ...
  methods: {
    ...
    qeuryList(id) {
      ...
      vm.api
        .get(
          `./manage/service/automation/queryTaskAssignLog/${id}&currentPage=${
            vm.currentPage
