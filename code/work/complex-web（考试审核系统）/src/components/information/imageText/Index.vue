...
export default {
  ...
  methods: {
    ...
    downLoadText() {
      location.href =
        location.origin + `/complex/publicservice/checkEntry/grahpics/template`;
    },
    ...
    handleAvatarSuccess(res, file) {
      let vm = this;
      this.api
        .get(`./complex/service/checkEntry/graphics/import?fId=${res[0].fId}`)
    ...
    exportList() {
      ...
      if (this.totals > 0) {
        ...
        this.api
          .post(`./complex/service/checkEntry/graphics/export`, data)
          .then(response => {
            if (response.status == 200) {
              vm.$confirm(
                "导入成功，前往【<a style='color: rgb(0, 205, 202);' href=" +
    ...
    getInformation() {
      ...
      this.api
        .post(
          `./complex/service/checkEntry/graphics/page?currentPage=${
            this.currentPage
