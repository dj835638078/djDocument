...
!((document, window) => {
  ...
  Vue.component("vue-slideshow", {
			...
			>
				<div class="v-slideshow-wrapper" :style="wrapperStyle">
					<div class="v-slideshow-box" v-if="config.effect === 'slide'" :style="boxStyle( 0 )">
						<a :href="data[ data.length - 1 ].href" style="position:absolute;width:2560px;height:380px;left: 50%;margin-left:-1280px;"><img :src="data[ data.length - 1 ].src"></a>
					</div>
					<div v-for="( list, index ) in data" :class="boxClass( index )" :style="boxStyle( index + 1 )">
						<a :href="list.href" style="position:absolute;width:2560px;height:380px;left: 50%;margin-left:-1280px;"><img :src="list.src"></a>
					</div>
					<div class="v-slideshow-box" v-if="config.effect === 'slide'" :style="boxStyle( data.length + 1 )">
						<a :href="data[ 0 ].href" style="position:absolute;width:2560px;height:380px;left: 50%;margin-left:-1280px;"><img :src="data[ 0 ].src"></a>
					</div>
				</div>
				<div class="v-slideshow-arrow" v-show="config.arrow !== false && !Array.isArray( config.arrow )">
					<i @click="arrowEvent( 0 )" style="z-index:101"></i>
					<i @click="arrowEvent( 1 )" style="z-index:101"></i>
				</div>
    ...
    methods: {
      ...
      enter: function () {
        $(".v-slideshow-arrow").style.display = "block"
        // console.log($(".v-slideshow-arrow").style.display)
        // this.$parent.config.arrow = true
        this.config.autoplay && window.clearInterval(window.VueSlideShowIDCache[this.$el.getAttribute("data-id")].autoTimer);
      },
      leave: function () {
        $(".v-slideshow-arrow").style.display = "none"
        // this.$parent.config.arrow = false
