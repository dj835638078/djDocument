<template>
  <div
    class="caseTrainDetail"
    :style="{ minHeight: detailHeight - 130 + 'px' }"
  >
    <div class="detailContent">
      <div class="detailTitle">
        <div class="title" style="height:33px;">
          {{ detailInfo.caseTitile }}
        </div>
      ...
      <div class="detailDes">
        <div class="infoSimpleLsft">
          ...
          <span style="color:#00cdca">{{
            tools.setErrorType(detailInfo.errorType)
          }}</span>
        </div>
        <div class="infoSimple">
          <img src="../../../static/user.svg" />
          <span class="author">{{ detailInfo.caseAuthor }}</span>
          <div class="line"></div>
          <span class="date">{{
            tools.setFormTime(detailInfo.releaseDate, 0)
          }}</span>
        </div>
        <div v-text="detailInfo.caseDesc" class="bulletinDesc"></div>
      </div>
      <div
        class="detailInfo"
        v-html="detailInfo.caseContent"
      ...
      >
        <div class="caseTitle" v-if="item.ruleContent">
          规则{{ tools.NumberToChinese(index + 1) }}：
        </div>
        <div
          class="detailInfo"
        ...
        <div class="caseTitle" v-else>
          案例{{ tools.NumberToChinese(index + 1) }}：
        </div>
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      let vm = this;
      vm.api
        .get(`./complex/service/webCases/find/${vm.$route.params.caseId}`)
        .then(response => {
